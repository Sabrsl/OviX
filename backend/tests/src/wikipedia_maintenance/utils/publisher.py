...
from .edit_summaries import get_summary
...
class Corrector:
    ...
    def get_side_by_side_diff(self) -> List[Tuple[str, str]]:
        ...
        For 'replace' opcodes, original and corrected lines are paired
        positionally up to the length of the shorter side; any surplus lines
        on the longer side are appended with None on the other column.
        """
        original_lines = self.original_content.splitlines()
        corrected_lines = self.current_content.splitlines()
        ...
        for op, i1, i2, j1, j2 in differ.get_opcodes():
            if op == 'equal':
                ...
                orig_slice = original_lines[i1:i2]
                corr_slice = corrected_lines[j1:j2]
                paired = min(len(orig_slice), len(corr_slice))
                for k in range(paired):
                    result.append((orig_slice[k], corr_slice[k]))
                for k in range(paired, len(orig_slice)):
                    result.append((orig_slice[k], None))
                for k in range(paired, len(corr_slice)):
                    result.append((None, corr_slice[k]))
            elif op == 'delete':
                for i in range(i1, i2):
...
class Publisher:
    ...
    def __init__(self, username: str = "LearnLynx", password: str = None, dry_run: bool = True, lang: str = "fr"):
        ...
        try:
            config_file = Path(__file__).parent.parent.parent.parent.parent.parent / "config" / "config.yaml"
    ...
    def generate_edit_summary(self, num_corrections: int, correction_types: List[str]) -> str:
        ...
        issue_types = dict(Counter(correction_types)) if correction_types else {}
        # issue_types (comptage) est l'entrée réellement utilisée par get_summary ;
        # correction_types n'est plus transmis pour éviter toute ambiguïté sur
        # quel argument pilote effectivement le résumé généré.
        return get_summary(corrections_count=num_corrections, issue_types=issue_types)
    ...
    def preview_changes(self, page_title: str, new_content: str) -> Tuple[bool, str, str]:
        ...
        """
        try:
            # Get current page content with modern API format
            ...
            # Extract content using modern API format (formatversion=2, rvslots=main)
            current_content = pages[page_id].get('revisions', [{}])[0].get('slots', {}).get('main', {}).get('content', '')
