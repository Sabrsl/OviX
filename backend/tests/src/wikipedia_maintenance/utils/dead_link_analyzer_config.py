"""
DeadLinkAnalyzer Configuration Loader.

This module handles loading configuration for the DeadLinkAnalyzer from YAML files.
It was extracted from the monolithic DeadLinkAnalyzer class to improve testability
and decouple the analyzer from the project folder structure.
...
_CONFIG_SECTION_KEY: Final[str] = 'dead_links_analyzer'


class DeadLinkAnalyzerConfig:
    """
    Configuration loader for DeadLinkAnalyzer.
    ...
    DEFAULT_ENABLE_AUTO_REPAIR: Final[bool] = True
    DEFAULT_ASSUME_WIKIPEDIA_PATCH_DEPLOYED: Final[bool] = False
    ...
    def load(cls, config_path: Optional[Path] = None) -> "DeadLinkAnalyzerConfig":
        ...
        Returns:
            DeadLinkAnalyzerConfig instance with loaded configuration
            (falls back to an all-defaults instance on any error).
        """
        if config_path is None:
            # Default path relative to this file
            config_path = Path(__file__).parent.parent.parent.parent.parent.parent / "config" / "config.yaml"

        config_data: Dict[str, Any] = {
            'timeout': cls.DEFAULT_TIMEOUT,
            'max_retries': cls.DEFAULT_MAX_RETRIES,
            'max_checks_per_article': cls.DEFAULT_MAX_CHECKS_PER_ARTICLE,
            'enable_auto_repair': cls.DEFAULT_ENABLE_AUTO_REPAIR,
            'assume_wikipedia_patch_deployed': cls.DEFAULT_ASSUME_WIKIPEDIA_PATCH_DEPLOYED,
        }
            ...
            try:
                ...
                if yaml_config and isinstance(yaml_config, dict) and _CONFIG_SECTION_KEY in yaml_config:
                    ...
                    if isinstance(analyzer_config, dict):
                        ...
                        logger.info(
                            f"Loaded config: timeout={config_data['timeout']}s, "
                            f"max_retries={config_data['max_retries']}, "
                            f"max_checks={config_data['max_checks_per_article']}, "
                            f"auto_repair={config_data['enable_auto_repair']}"
                        )
                    else:
                        logger.warning(
    ...
    def __init__(self, timeout: int = DEFAULT_TIMEOUT, max_retries: int = DEFAULT_MAX_RETRIES,
                 ...
                 enable_auto_repair: bool = DEFAULT_ENABLE_AUTO_REPAIR,
                 assume_wikipedia_patch_deployed: bool = DEFAULT_ASSUME_WIKIPEDIA_PATCH_DEPLOYED):
        """
        ...
        Args:
            ...
            enable_auto_repair: Whether to automatically repair dead links
            assume_wikipedia_patch_deployed: Whether to assume Wikipedia Lua patch is deployed
        """
        self.timeout = timeout
        self.max_retries = max_retries
        self.max_checks_per_article = max_checks_per_article
        self.enable_auto_repair = enable_auto_repair
        self.assume_wikipedia_patch_deployed = assume_wikipedia_patch_deployed
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'assume_wikipedia_patch_deployed': self.assume_wikipedia_patch_deployed,
        }
    ...
    def __repr__(self) -> str:
        return f"DeadLinkAnalyzerConfig({self.to_dict()})"