...
class DomainEnrichmentService:
    ...
    CASE_NORMALIZATION_FILE = Path(__file__).parent.parent.parent.parent.parent / "config" / "case_normalization_data.yaml"
    ANALYSIS_TRACKER_FILE = Path(__file__).parent.parent.parent.parent.parent / "data" / "domain_enrichment_tracker.json"
    RESULTS_FILE = Path(__file__).parent.parent.parent.parent.parent / "data" / "domain_enrichment_results.json"
    ...
    def reset(self) -> None:
        ...
        try:
            state_file = Path(__file__).parent.parent.parent.parent.parent / "data" / "domain_enrichment_pagination.json"
    ...
    def _process_page(self, page_title: str, wiki_lang: str, keep_www: bool = False, max_retries: int = 3) -> Optional[DomainEntry]:
        """
        Process a single Wikipedia page to extract domain mappings with retry logic.
        ...
        for attempt in range(max_retries):
            ...
            if self._cancel_event.is_set():
                logger.debug(f"Skipping page {page_title} due to cancellation")
                return None
            ...
            try:
                ...
                # Check if it's a network-related error that might be retriable
                error_msg = str(e).lower()
                is_network_error = any(keyword in error_msg for keyword in [
                    'timeout', 'connection', 'network', 'http', 'ssl', 'tls',
                    'temporary failure', 'service unavailable', '503', '502', '504'
                ])
                
                if is_network_error and attempt < max_retries - 1:
                    logger.warning(f"Network error processing page {page_title} (attempt {attempt + 1}/{max_retries}): {e}. Retrying...")
                    # Sleep with cancellation checks
                    sleep_time = 2 ** attempt
                    for _ in range(int(sleep_time * 10)):  # Check every 0.1s
                        if self._cancel_event.is_set():
                            logger.debug(f"Cancellation during retry sleep for page {page_title}")
                            return None
                        time.sleep(0.1)
                    if self._cancel_event.is_set():
                        return None
                    ...
                    logger.error(f"Error processing page {page_title} (attempt {attempt + 1}/{max_retries}): {e}")
                    with self._lock:
                        self._progress.errors += 1
                    self._mark_article_analyzed(page_title, 'error')
                    return None

    def _process_category(self, category: str, wiki_lang: str, batch_size: int = 500, max_pages: Optional[int] = None, keep_www: bool = False, max_retries: int = 3) -> List[DomainEntry]:
        """
        ...
        """
        # Defensive check: ensure wiki_lang is not a namespace prefix
        namespace_prefixes = ['catégorie', 'category', 'wikipedia', 'file', 'image', 'template', 'modèle']
        if wiki_lang.lower() in namespace_prefixes:
            logger.warning(f"Invalid wiki_lang '{wiki_lang}' (appears to be a namespace prefix), defaulting to 'fr'")
            wiki_lang = 'fr'
        ...
        try:
            while has_more:
                ...
                for fetch_attempt in range(max_retries):
                    # Check for cancellation before each retry attempt
                    if self._cancel_event.is_set():
                        logger.info("Cancellation requested during retry loop, stopping")
                        with self._lock:
                            self._pagination_state[category_key] = {
                                'offset': offset,
                                'total_fetched': total_fetched,
                    ...
                    try:
                        ...
                        if is_network_error and fetch_attempt < max_retries - 1:
                            logger.warning(f"Network error fetching category members for '{category}' (attempt {fetch_attempt + 1}/{max_retries}): {e}. Retrying...")
                            # Sleep with cancellation checks
                            sleep_time = 2 ** fetch_attempt
                            for _ in range(int(sleep_time * 10)):  # Check every 0.1s
                                if self._cancel_event.is_set():
                                    logger.info("Cancellation requested during retry sleep, stopping")
                                    with self._lock:
                                        self._pagination_state[category_key] = {
                                            'offset': offset,
                                            'total_fetched': total_fetched,
                            ...
                            logger.error(f"Error fetching category members for '{category}' (attempt {fetch_attempt + 1}/{max_retries}): {e}")
                            with self._lock:
                                self._progress.errors += 1
                            # Treat as empty batch to continue to next category
                            members = []
                            break
                ...
                for member in members:
                    ...
                    # Strip namespace prefix if present (e.g., "Catégorie:" or "Category:")
                    # to avoid using it as wiki_lang in get_qid_from_page_title
                    clean_page_title = page_title
                    if ':' in page_title:
                        # Check if it's a namespace prefix (first part before colon)
                        parts = page_title.split(':', 1)
                        potential_namespace = parts[0].lower()
                        # Common namespaces to strip
                        if potential_namespace in ['catégorie', 'category', 'wikipedia', 'file', 'image', 'template', 'modèle']:
                            clean_page_title = parts[1]

                    # Process the page
                    entry = self._process_page(clean_page_title, wiki_lang, keep_www=keep_www)
                    if entry:
    ...
    def start_enrichment(
        ...
        Args:
            ...
            sequential_write: If True, write to YAML after each category (robust against crashes)
        ...
        logger.info(f"Starting enrichment with {len(categories)} categories (dry_run={dry_run}, max_pages={max_pages}, sequential_write={sequential_write})")
        ...
        try:
            ...
            for category_config in sorted_categories:
                ...
                # Get new entries from this category
                new_entries = [e for e in entries if e.status == EntryStatus.NEW]
                
                with self._lock:
                    self._entries.extend(entries)
                    new_domains_in_category = len(new_entries)

                # Sequential write: write immediately after each category
                if sequential_write and not dry_run and new_entries:
                    logger.info(f"Sequential write: Writing {new_domains_in_category} new domains from category '{category_config.category}'")
                    try:
                        success, written = self.write_to_yaml(new_entries)
                        if success:
                            logger.info(f"Sequential write successful: {written} entries written to YAML")
                            with self._lock:
                                self._progress.add_activity(f"✓ Category '{category_config.category}': {written} entries written to YAML")
                        else:
                            logger.error(f"Sequential write failed for category '{category_config.category}'")
                            with self._lock:
                                self._progress.add_activity(f"✗ Category '{category_config.category}': Write failed (entries kept in memory)")
                    except Exception as e:
                        logger.error(f"Error during sequential write for category '{category_config.category}': {e}")
                        with self._lock:
                            self._progress.add_activity(f"✗ Category '{category_config.category}': Write error (entries kept in memory)")
                        # Continue to next category despite write failure
                elif sequential_write and dry_run:
                    logger.info(f"Sequential write: Dry run mode, skipping write for category '{category_config.category}'")
                    with self._lock:
                        self._progress.add_activity(f"○ Category '{category_config.category}': {new_domains_in_category} new entries (dry run)")
                elif not sequential_write:
                    logger.info(f"Sequential write disabled, entries kept in memory for category '{category_config.category}'")
