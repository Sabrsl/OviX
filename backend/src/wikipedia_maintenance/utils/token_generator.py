...
def generate_tokens(
    ...
    """
    print(f"🔐 Génération de tokens sécurisés pour {operator_name}")
    print(f"⏰ Expiration : {expiry_hours} heures")
    print("-" * 50)
...
def display_token_info(token_info: dict, action: str) -> None:
    ...
    """
    print(f"\n🔗 Lien sécurisé pour {action.upper()} :")
    print("-" * 50)
    ...
    print("\n⚠️  IMPORTANT :")
    print("- Ce lien est à usage unique")
    print("- Il expire après 24 heures")
    print("- Conservez le token secret en sécurité")
    print("- Ne partagez pas ce lien publiquement")
...
def generate_wiki_markup(
    ...
    """
    if token_info:
        stop_link = f"[{token_info['stop']['url']} 🛑 ARRÊTER OviX]"
        resume_link = f"[{token_info['resume']['url']} ▶️ REPRIRE]"
    ...
    if args.generate_wiki:
        ...
        if args.output:
            with open(args.output, 'w', encoding='utf-8') as f:
                f.write(wiki_markup)
            print(f"\n✅ Markup sauvegardé dans : {args.output}")
        
        print("\n📋 Instructions :")
        print("1. Copiez ce markup")
        print("2. Collez-le sur la page de discussion du bot")
        print("3. Les liens sécurisés seront actifs immédiatement")
