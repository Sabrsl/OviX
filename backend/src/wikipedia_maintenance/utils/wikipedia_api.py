...
class WikipediaAPIClient:
    ...
    def __init__(
        ...
        if user_agent is None:
            try:
                ...
                self.user_agent = 'OviX/1.0 (+https://github.com/Sabrsl/OviX)'
        else:
    ...
    def get_category_members(
        ...
        """
        Get members of a category with pagination support.
        ...
        try:
            while True:
                ...
                limit = min(batch_size, max_results - total_fetched) if max_results else batch_size
                ...
                batch_members = data.get('query', {}).get('categorymembers', [])
                ...
                if not cmcontinue:
                    logger.info(f"Fetched all {len(members)} members from category '{category_name}'")
                    break
                
                logger.debug(f"Fetched {len(batch_members)} members, continuing with cmcontinue={cmcontinue}")
