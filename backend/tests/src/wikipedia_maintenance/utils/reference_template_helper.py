...
class ReferenceTemplateHelper:
    ...
    def _load_domain_to_site_name_mapping(cls) -> Dict[str, str]:
        ...
        """
        try:
            # Go from backend/tests/src/wikipedia_maintenance/utils/ to project root
            # Path: backend/tests/src/wikipedia_maintenance/utils -> backend/tests/src -> backend/tests -> backend -> project root
            config_path = Path(__file__).parent.parent.parent.parent.parent.parent / "config" / "case_normalization_data.yaml"
    ...
    def generate_archive_repair_template(
        ...
        if site_not_applicable:
            ...
            # Check if existing_site is a domain or URL that should be mapped
            # This includes cases like "lapresse.ca", "www.lapresse.ca", or full URLs
            if existing_site and ('://' in existing_site or existing_site.strip().startswith('www.') or '.' in existing_site.strip()):
                from urllib.parse import urlparse
                ...
                if corrected_site and corrected_site != existing_site.strip():
                    self._logger.info(f"ARCHIVE_SITE_CORRECTION | template={original_template.template_name} | existing_site={existing_site} | new_site={corrected_site} | reason=domain_mapping_applied")
                    params['site'] = corrected_site
                # If no mapping found but www. was present, still remove www. prefix (plain domain)
                elif existing_site.strip().startswith('www.') and domain != existing_site.strip():
                ...
                # Not a domain/URL, try to resolve anyway (will return unchanged if not a domain)
    ...
    def _resolve_site_display_name(self, domain: str) -> str:
        ...
        If the input is not a domain (e.g., already a human-readable name or wikilink),
        ...
        # If it contains brackets or looks like a name, return as-is
        if '[' in domain and ']' in domain:
            return domain
        
        # Check if it looks like a domain (contains dots and no spaces)
        # This prevents treating names like "Le Monde" as domains
        ...
        # YAML entries are in lowercase, so we need to match case-insensitively
        domain_lower = domain.lower()

        # Try exact match first (with lowercase normalization)
        ...
        # Try without www prefix (with lowercase normalization)
    ...
    def generate_enriched_template(
        ...
        if site_needs_correction:
            ...
            if corrected_site and corrected_site != current_site.strip():
                self._logger.info(f"ENRICHMENT_SITE_CORRECTION | template={original_template.template_name} | existing_site={current_site} | new_site={corrected_site} | reason=domain_mapping_applied")
                params['site'] = corrected_site
            # If no mapping found but www. was present, still remove www. prefix (plain domain)
