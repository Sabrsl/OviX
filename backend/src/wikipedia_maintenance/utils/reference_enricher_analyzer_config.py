...
class ReferenceEnricherConfig:
    ...
    def validate(self) -> bool:
        ...
        try:
            ...
            if not isinstance(self.enable_site_fill, bool):
                logger.error(f"Invalid enable_site_fill: {self.enable_site_fill} must be boolean")
                return False

            if not isinstance(self.enable_consulte_le_fill, bool):
                logger.error(f"Invalid enable_consulte_le_fill: {self.enable_consulte_le_fill} must be boolean")
                return False
